/*
 * SOLIDserver API
 *
 * OpenAPI 3.0.2 API definition for SOLIDserver service from EfficientIP.<p>Copyright © 2000-2021 EfficientIP</p><p><em>All specifications and information regarding the products in  this document are subject to change without notice and should not be  construed as a commitment by EfficientIP. EfficientIP assumes no  responsibility or liability for any mistakes or inaccuracies that may appear  in this document. All statements and recommendations in this document are  believed to be accurate but are presented without warranty. Users must take  full responsibility for their application of any product.</em></p><p>Generated (Monday 14th of June 2021 12:30:34 PM)</p>
 *
 * API version: 2.0
 * Contact: support-api@efficientip.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sdsclient

import (
	"encoding/json"
)

// GuardianPolicyDataData struct for GuardianPolicyDataData
type GuardianPolicyDataData struct {
	// The delay of creation/deletion status. <b>1</b> indicates that the object is not created/deleted yet.
	PolicyDelayedTime *string `json:"policy_delayed_time,omitempty"`
	// The database identifier (ID) of the Guardian server associated with the policy. It is only returned for deployed policies.
	ServerId *string `json:"server_id,omitempty"`
	// The name of the Guardian server associated with the policy. It is only returned for deployed policies.
	ServerName *string `json:"server_name,omitempty"`
	// The description of the policy.
	PolicyDescription *string `json:"policy_description,omitempty"`
	// The database identifier (ID) of the policy.
	PolicyId *string `json:"policy_id,omitempty"`
	// The name of the policy.
	PolicyName *string `json:"policy_name,omitempty"`
	// The read only status of the policy. If set to <b>1</b>, the policy cannot be edited.
	PolicyReadonly *string `json:"policy_readonly,omitempty"`
	// The database identifier (ID) of the policy. It is only returned for deployed policies.
	ParentPolicyId *string `json:"parent_policy_id,omitempty"`
}

// NewGuardianPolicyDataData instantiates a new GuardianPolicyDataData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGuardianPolicyDataData() *GuardianPolicyDataData {
	this := GuardianPolicyDataData{}
	return &this
}

// NewGuardianPolicyDataDataWithDefaults instantiates a new GuardianPolicyDataData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGuardianPolicyDataDataWithDefaults() *GuardianPolicyDataData {
	this := GuardianPolicyDataData{}
	return &this
}

// GetPolicyDelayedTime returns the PolicyDelayedTime field value if set, zero value otherwise.
func (o *GuardianPolicyDataData) GetPolicyDelayedTime() string {
	if o == nil || o.PolicyDelayedTime == nil {
		var ret string
		return ret
	}
	return *o.PolicyDelayedTime
}

// GetPolicyDelayedTimeOk returns a tuple with the PolicyDelayedTime field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuardianPolicyDataData) GetPolicyDelayedTimeOk() (*string, bool) {
	if o == nil || o.PolicyDelayedTime == nil {
		return nil, false
	}
	return o.PolicyDelayedTime, true
}

// HasPolicyDelayedTime returns a boolean if a field has been set.
func (o *GuardianPolicyDataData) HasPolicyDelayedTime() bool {
	if o != nil && o.PolicyDelayedTime != nil {
		return true
	}

	return false
}

// SetPolicyDelayedTime gets a reference to the given string and assigns it to the PolicyDelayedTime field.
func (o *GuardianPolicyDataData) SetPolicyDelayedTime(v string) {
	o.PolicyDelayedTime = &v
}

// GetServerId returns the ServerId field value if set, zero value otherwise.
func (o *GuardianPolicyDataData) GetServerId() string {
	if o == nil || o.ServerId == nil {
		var ret string
		return ret
	}
	return *o.ServerId
}

// GetServerIdOk returns a tuple with the ServerId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuardianPolicyDataData) GetServerIdOk() (*string, bool) {
	if o == nil || o.ServerId == nil {
		return nil, false
	}
	return o.ServerId, true
}

// HasServerId returns a boolean if a field has been set.
func (o *GuardianPolicyDataData) HasServerId() bool {
	if o != nil && o.ServerId != nil {
		return true
	}

	return false
}

// SetServerId gets a reference to the given string and assigns it to the ServerId field.
func (o *GuardianPolicyDataData) SetServerId(v string) {
	o.ServerId = &v
}

// GetServerName returns the ServerName field value if set, zero value otherwise.
func (o *GuardianPolicyDataData) GetServerName() string {
	if o == nil || o.ServerName == nil {
		var ret string
		return ret
	}
	return *o.ServerName
}

// GetServerNameOk returns a tuple with the ServerName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuardianPolicyDataData) GetServerNameOk() (*string, bool) {
	if o == nil || o.ServerName == nil {
		return nil, false
	}
	return o.ServerName, true
}

// HasServerName returns a boolean if a field has been set.
func (o *GuardianPolicyDataData) HasServerName() bool {
	if o != nil && o.ServerName != nil {
		return true
	}

	return false
}

// SetServerName gets a reference to the given string and assigns it to the ServerName field.
func (o *GuardianPolicyDataData) SetServerName(v string) {
	o.ServerName = &v
}

// GetPolicyDescription returns the PolicyDescription field value if set, zero value otherwise.
func (o *GuardianPolicyDataData) GetPolicyDescription() string {
	if o == nil || o.PolicyDescription == nil {
		var ret string
		return ret
	}
	return *o.PolicyDescription
}

// GetPolicyDescriptionOk returns a tuple with the PolicyDescription field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuardianPolicyDataData) GetPolicyDescriptionOk() (*string, bool) {
	if o == nil || o.PolicyDescription == nil {
		return nil, false
	}
	return o.PolicyDescription, true
}

// HasPolicyDescription returns a boolean if a field has been set.
func (o *GuardianPolicyDataData) HasPolicyDescription() bool {
	if o != nil && o.PolicyDescription != nil {
		return true
	}

	return false
}

// SetPolicyDescription gets a reference to the given string and assigns it to the PolicyDescription field.
func (o *GuardianPolicyDataData) SetPolicyDescription(v string) {
	o.PolicyDescription = &v
}

// GetPolicyId returns the PolicyId field value if set, zero value otherwise.
func (o *GuardianPolicyDataData) GetPolicyId() string {
	if o == nil || o.PolicyId == nil {
		var ret string
		return ret
	}
	return *o.PolicyId
}

// GetPolicyIdOk returns a tuple with the PolicyId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuardianPolicyDataData) GetPolicyIdOk() (*string, bool) {
	if o == nil || o.PolicyId == nil {
		return nil, false
	}
	return o.PolicyId, true
}

// HasPolicyId returns a boolean if a field has been set.
func (o *GuardianPolicyDataData) HasPolicyId() bool {
	if o != nil && o.PolicyId != nil {
		return true
	}

	return false
}

// SetPolicyId gets a reference to the given string and assigns it to the PolicyId field.
func (o *GuardianPolicyDataData) SetPolicyId(v string) {
	o.PolicyId = &v
}

// GetPolicyName returns the PolicyName field value if set, zero value otherwise.
func (o *GuardianPolicyDataData) GetPolicyName() string {
	if o == nil || o.PolicyName == nil {
		var ret string
		return ret
	}
	return *o.PolicyName
}

// GetPolicyNameOk returns a tuple with the PolicyName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuardianPolicyDataData) GetPolicyNameOk() (*string, bool) {
	if o == nil || o.PolicyName == nil {
		return nil, false
	}
	return o.PolicyName, true
}

// HasPolicyName returns a boolean if a field has been set.
func (o *GuardianPolicyDataData) HasPolicyName() bool {
	if o != nil && o.PolicyName != nil {
		return true
	}

	return false
}

// SetPolicyName gets a reference to the given string and assigns it to the PolicyName field.
func (o *GuardianPolicyDataData) SetPolicyName(v string) {
	o.PolicyName = &v
}

// GetPolicyReadonly returns the PolicyReadonly field value if set, zero value otherwise.
func (o *GuardianPolicyDataData) GetPolicyReadonly() string {
	if o == nil || o.PolicyReadonly == nil {
		var ret string
		return ret
	}
	return *o.PolicyReadonly
}

// GetPolicyReadonlyOk returns a tuple with the PolicyReadonly field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuardianPolicyDataData) GetPolicyReadonlyOk() (*string, bool) {
	if o == nil || o.PolicyReadonly == nil {
		return nil, false
	}
	return o.PolicyReadonly, true
}

// HasPolicyReadonly returns a boolean if a field has been set.
func (o *GuardianPolicyDataData) HasPolicyReadonly() bool {
	if o != nil && o.PolicyReadonly != nil {
		return true
	}

	return false
}

// SetPolicyReadonly gets a reference to the given string and assigns it to the PolicyReadonly field.
func (o *GuardianPolicyDataData) SetPolicyReadonly(v string) {
	o.PolicyReadonly = &v
}

// GetParentPolicyId returns the ParentPolicyId field value if set, zero value otherwise.
func (o *GuardianPolicyDataData) GetParentPolicyId() string {
	if o == nil || o.ParentPolicyId == nil {
		var ret string
		return ret
	}
	return *o.ParentPolicyId
}

// GetParentPolicyIdOk returns a tuple with the ParentPolicyId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuardianPolicyDataData) GetParentPolicyIdOk() (*string, bool) {
	if o == nil || o.ParentPolicyId == nil {
		return nil, false
	}
	return o.ParentPolicyId, true
}

// HasParentPolicyId returns a boolean if a field has been set.
func (o *GuardianPolicyDataData) HasParentPolicyId() bool {
	if o != nil && o.ParentPolicyId != nil {
		return true
	}

	return false
}

// SetParentPolicyId gets a reference to the given string and assigns it to the ParentPolicyId field.
func (o *GuardianPolicyDataData) SetParentPolicyId(v string) {
	o.ParentPolicyId = &v
}

func (o GuardianPolicyDataData) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.PolicyDelayedTime != nil {
		toSerialize["policy_delayed_time"] = o.PolicyDelayedTime
	}
	if o.ServerId != nil {
		toSerialize["server_id"] = o.ServerId
	}
	if o.ServerName != nil {
		toSerialize["server_name"] = o.ServerName
	}
	if o.PolicyDescription != nil {
		toSerialize["policy_description"] = o.PolicyDescription
	}
	if o.PolicyId != nil {
		toSerialize["policy_id"] = o.PolicyId
	}
	if o.PolicyName != nil {
		toSerialize["policy_name"] = o.PolicyName
	}
	if o.PolicyReadonly != nil {
		toSerialize["policy_readonly"] = o.PolicyReadonly
	}
	if o.ParentPolicyId != nil {
		toSerialize["parent_policy_id"] = o.ParentPolicyId
	}
	return json.Marshal(toSerialize)
}

type NullableGuardianPolicyDataData struct {
	value *GuardianPolicyDataData
	isSet bool
}

func (v NullableGuardianPolicyDataData) Get() *GuardianPolicyDataData {
	return v.value
}

func (v *NullableGuardianPolicyDataData) Set(val *GuardianPolicyDataData) {
	v.value = val
	v.isSet = true
}

func (v NullableGuardianPolicyDataData) IsSet() bool {
	return v.isSet
}

func (v *NullableGuardianPolicyDataData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGuardianPolicyDataData(val *GuardianPolicyDataData) *NullableGuardianPolicyDataData {
	return &NullableGuardianPolicyDataData{value: val, isSet: true}
}

func (v NullableGuardianPolicyDataData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGuardianPolicyDataData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


