...
// APIClient manages communication with the SOLIDserver API API v2.0
...
type APIClient struct {
	...
	GuardianAPI *GuardianAPIService
...
func NewAPIClient(cfg *Configuration) *APIClient {
	...
	c.GuardianAPI = (*GuardianAPIService)(&c.common)
...
func (c *APIClient) prepareRequest(
	...
	if ctx != nil {
		...
		// EipApiToken Authentication
		if auth, ok := ctx.Value(ContextEipApiTokenAuth).(EipApiTokenAuth); ok {
			timestamp := time.Now().Unix()
			signature := auth.GenerateSignature(localVarRequest, timestamp)
			localVarRequest.Header.Add("Authorization", fmt.Sprintf("SDS %s:%x", auth.Token, signature))
			localVarRequest.Header.Add("X-SDS-TS", fmt.Sprintf("%d", timestamp))
		}
	}
